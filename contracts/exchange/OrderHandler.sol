...
contract OrderHandler is IOrderHandler, BaseOrderHandler {
    ...
    ) BaseOrderHandler(_roleStore, _dataStore, _eventEmitter, _orderVault, _oracle, _swapHandler, _referralStorage) {}
    ...
    function createOrder(
        ...
        FeatureUtils.validateFeature(
            dataStore,
            Keys.createOrderFeatureDisabledKey(address(this), uint256(params.orderType))
        );
    ...
    function updateOrder(
        ...
        FeatureUtils.validateFeature(
            dataStore,
            Keys.updateOrderFeatureDisabledKey(address(this), uint256(order.orderType()))
        );
    ...
    function cancelOrder(bytes32 key) external override globalNonReentrant onlyController {
        ...
        FeatureUtils.validateFeature(
            _dataStore,
            Keys.cancelOrderFeatureDisabledKey(address(this), uint256(order.orderType()))
        );
    ...
    ) external override onlyController withSimulatedOraclePrices(oracle, params) globalNonReentrant {
        OracleUtils.SetPricesParams memory oracleParams;
        Order.Props memory order = OrderStoreUtils.get(dataStore, key);

        this._executeOrder(key, order, oracleParams, msg.sender);
    }
    ...
    function executeOrder(
    ...
    ) external globalNonReentrant onlyOrderKeeper withOraclePrices(oracle, dataStore, eventEmitter, oracleParams) {
        uint256 startingGas = gasleft();
        ...
        try this._executeOrder{gas: executionGas}(key, order, oracleParams, msg.sender) {} catch (
    ...
    function _executeOrder(
        ...
        FeatureUtils.validateFeature(
            ...
            Keys.executeOrderFeatureDisabledKey(address(this), uint256(params.order.orderType()))
        );
    ...
    function _handleOrderError(bytes32 key, uint256 startingGas, bytes memory reasonBytes) internal {
        ...
        OrderUtils.freezeOrder(dataStore, eventEmitter, orderVault, key, msg.sender, startingGas, reason, reasonBytes);
    }
