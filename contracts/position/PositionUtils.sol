...
library PositionUtils {
    ...
    function getPositionPnlUsd(
        ...
        //计算仓位的pnl
        cache.totalPositionPnl = position.isLong()
            ? cache.positionValue - position.sizeInUsd().toInt256()
            : position.sizeInUsd().toInt256() - cache.positionValue;
        ...
        if (cache.totalPositionPnl > 0) {
            ...
            cache.poolPnl = MarketUtils.getPnl(dataStore, market, prices.indexTokenPrice, position.isLong(), true);
            ...
            if (cache.cappedPoolPnl != cache.poolPnl && cache.cappedPoolPnl > 0 && cache.poolPnl > 0) {
                cache.totalPositionPnl = Precision.mulDiv(
                    cache.totalPositionPnl.toUint256(),
                    cache.cappedPoolPnl,
        ...
        if (position.sizeInUsd() == sizeDeltaUsd) {
            ...
            if (position.isLong()) {
                cache.sizeDeltaInTokens = Calc.roundUpDivision(
                    position.sizeInTokens() * sizeDeltaUsd,
                    position.sizeInUsd()
                );
            } else {
        ...
        //根据用户总size 总pnl 和减仓size 算出 pnl
        cache.positionPnlUsd = Precision.mulDiv(
            cache.totalPositionPnl,
            cache.sizeDeltaInTokens,
            position.sizeInTokens()
        );
        cache.uncappedPositionPnlUsd = Precision.mulDiv(
            cache.uncappedTotalPositionPnl,
            cache.sizeDeltaInTokens,
            position.sizeInTokens()
        );
    ...
    function isPositionLiquidatable(
        ...
        cache.collateralTokenPrice = MarketUtils.getCachedTokenPrice(position.collateralToken(), market, prices);
        ...
        if (cache.priceImpactUsd >= 0) {
            ...
            int256 maxNegativePriceImpactUsd = -Precision
                .applyFactor(position.sizeInUsd(), maxPriceImpactFactor)
                .toInt256();
        ...
        info.minCollateralUsdForLeverage = Precision
            .applyFactor(position.sizeInUsd(), cache.minCollateralFactor)
            .toInt256();
    ...
    function willPositionCollateralBeSufficient(
        ...
        Price.Props memory collateralTokenPrice = MarketUtils.getCachedTokenPrice(collateralToken, market, prices);
        ...
        int256 minCollateralUsdForLeverage = Precision
            .applyFactor(values.positionSizeInUsd, minCollateralFactor)
            .toInt256();
    ...
    function getExecutionPriceForIncrease(
        ...
        //对于多单 如果price impact 是正的 sizeDeltaInTokens+=priceImpactAmount* minPrice
        //对于多单 如果price impact 是负的 sizeDeltaInTokens-=priceImpactAmount* maxPrice
        //对于空单 如果price impact 是正的 sizeDeltaInTokens-=priceImpactAmount* minPrice
        //对于空单 如果price impact 是负的 sizeDeltaInTokens+=priceImpactAmount* maxPrice
