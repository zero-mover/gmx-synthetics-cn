...
library MarketUtils {
    ...
    function getMarketTokenPrice(
        ...
        if (poolValueInfo.poolValue == 0) {
            return (0, poolValueInfo);
        }
        //根据池子的金额和LP Token数量计算一下LP的价格
    ...
    // @dev 获取池子的价值多少美元 忽略impact price 因为所有的仓位全平之后 price impact 为 0
    ...
    function getPoolValueInfo(
        ...
        //将三种代币价格进行封装
        MarketPrices memory prices = MarketPrices(indexTokenPrice, longTokenPrice, shortTokenPrice);
        // 获取 多空的borrow fees
        result.totalBorrowingFees = getTotalPendingBorrowingFees(dataStore, market, prices, true);

        result.totalBorrowingFees += getTotalPendingBorrowingFees(dataStore, market, prices, false);

        result.borrowingFeePoolFactor =
        ...
        //如果 maximize为true   计算池子的金额大
        //    !maximize为false 计算pnl就是最小的
        //
    ...
    function getPnl(
        ...
        return getPnl(dataStore, market, _indexTokenPrice, isLong, maximize);
    }
    ...
    function getMaxPoolAmountForDeposit(
        DataStore dataStore,
        address market,
        address token
    ) internal view returns (uint256) {
    ...
    function incrementClaimableFundingAmount(
        ...
        uint256 nextValue = dataStore.incrementUint(Keys.claimableFundingAmountKey(market, token, account), delta);

        uint256 nextPoolValue = dataStore.incrementUint(Keys.claimableFundingAmountKey(market, token), delta);
    ...
    function claimFundingFees(
        ...
        uint256 nextPoolValue = dataStore.decrementUint(Keys.claimableFundingAmountKey(market, token), claimableAmount);

        MarketToken(payable(market)).transferOut(token, receiver, claimableAmount);
    ...
    function applyDeltaToPoolAmount(
        ...
        applyDeltaToVirtualInventoryForSwaps(dataStore, eventEmitter, market, token, delta);
    ...
    function getAdjustedPositionImpactFactor(
        ...
        (uint256 positiveImpactFactor, uint256 negativeImpactFactor) = getAdjustedPositionImpactFactors(
            dataStore,
            market
        );
    ...
    function getCappedPositionImpactUsd(
        ...
        int256 maxPriceImpactUsdBasedOnMaxPriceImpactFactor = Precision
            .applyFactor(sizeDeltaUsd, maxPriceImpactFactor)
            .toInt256();
    ...
    function getSwapImpactPoolAmount(
        DataStore dataStore,
        address market,
        address token
    ) internal view returns (uint256) {
    ...
    function applyDeltaToSwapImpactPool(
        ...
        uint256 nextValue = dataStore.applyBoundedDeltaToUint(Keys.swapImpactPoolAmountKey(market, token), delta);
    ...
    function applyDeltaToPositionImpactPool(
        ...
        uint256 nextValue = dataStore.applyBoundedDeltaToUint(Keys.positionImpactPoolAmountKey(market), delta);
    ...
    function applyDeltaToOpenInterest(
        ...
        //开多 是从 virtual inventory 中购买(从中借出) 因此 virtual inventory 应该减少
        //关多 是将 借出的size 还给 virtual inventory  因此 virtual inventory 应该增加
        //开空 应该是以现在价格卖给 virtual inventory
        //关空
        ...
        // so the virtual inventory should be decreased
        applyDeltaToVirtualInventoryForPositions(dataStore, eventEmitter, market.indexToken, isLong ? -delta : delta);

        if (delta > 0) {
            validateOpenInterest(dataStore, market, isLong);
        }

        MarketEventUtils.emitOpenInterestUpdated(
            eventEmitter,
            market.marketToken,
            collateralToken,
            isLong,
    ...
    function applyDeltaToOpenInterestInTokens(
        ...
        MarketEventUtils.emitOpenInterestInTokensUpdated(
            eventEmitter,
            market,
            collateralToken,
            isLong,
    ...
    function getNextFundingAmountPerSize(
        ...
        cache.openInterest.long.longToken = getOpenInterest(
            dataStore,
            market.marketToken,
            market.longToken,
            true,
        ...
        cache.openInterest.long.shortToken = getOpenInterest(
            dataStore,
            market.marketToken,
            market.shortToken,
            true,
        ...
        cache.openInterest.short.longToken = getOpenInterest(
            dataStore,
            market.marketToken,
            market.longToken,
            false,
        ...
        cache.openInterest.short.shortToken = getOpenInterest(
            dataStore,
            market.marketToken,
            market.shortToken,
            false,
        ...
        //获取大的一方开仓的size
        cache.sizeOfLargerSide = cache.longOpenInterest > cache.shortOpenInterest
        ...
        (
            ...
            result.nextSavedFundingFactorPerSecond
        ...
        /*
        对于只有一种token的market 资金费会被重新计算
        - 假如 
            -- long open interset  价值 $200,000
            -- short open interest 价值 $100,000
            -- 资金费 8刀
        ...
        */
        ...
        // fundingFeeAmountPerSizeDelta要除以100,000的原因是  
        // cache.openInterest.long.longOpenInterest 和 cache.openInterest.long.shortOpenInterest 
        // 要除以2
        ...
        //为了防止longToken和shortToken是一种代币导致的重复计算
        ...
        if (result.longsPayShorts) {
            cache.fundingUsdForLongCollateral = Precision.mulDiv(
                cache.fundingUsd,
                cache.openInterest.long.longToken,
                cache.longOpenInterest
            );
            cache.fundingUsdForShortCollateral = Precision.mulDiv(
                cache.fundingUsd,
    ...
    function getNextFundingFactorPerSecond(
        ...
        //pow(diffUsd,fundingExponentFactor)
        ...
        //如果 fundingIncreaseFactorPerSecond 为0
        ...
        //多空比是否发生翻转 true 代表未发生翻转
        bool isSkewTheSameDirectionAsFunding = (cache.savedFundingFactorPerSecond > 0 &&
            longOpenInterest > shortOpenInterest) ||
            (cache.savedFundingFactorPerSecond < 0 && shortOpenInterest > longOpenInterest);
        ...
        if (isSkewTheSameDirectionAsFunding) {
            ...
            if (cache.diffUsdToOpenInterestFactor > configCache.thresholdForStableFunding) {
                ...
                //不平衡因子小于 thresholdForDecreaseFunding
                fundingRateChangeType = FundingRateChangeType.Decrease;
            }
            //如果未发生翻转并且 不平衡因子在thresholdForDecreaseFunding 和 thresholdForStableFunding 之间 则不做任何
        } else {
        ...
        if (fundingRateChangeType == FundingRateChangeType.Increase) {
            ...
            int256 increaseValue = Precision
                .applyFactor(cache.diffUsdToOpenInterestFactor, configCache.fundingIncreaseFactorPerSecond)
                .toInt256() * durationInSeconds.toInt256();
            ...
            // 如果 开多大于开空 savedFundingFactorPerSecond 应该增加
            //否则应该减少
        ...
        if (
            fundingRateChangeType == FundingRateChangeType.Decrease && cache.savedFundingFactorPerSecondMagnitude != 0
        ) {
            configCache.fundingDecreaseFactorPerSecond = dataStore.getUint(
                Keys.fundingDecreaseFactorPerSecondKey(market)
            );
            ...
            if (cache.savedFundingFactorPerSecondMagnitude <= decreaseValue) {
                ...
                cache.nextSavedFundingFactorPerSecond =
                    ...
                    cache.savedFundingFactorPerSecondMagnitude.toInt256();
            } else {
                ...
                cache.nextSavedFundingFactorPerSecond =
                    (cache.savedFundingFactorPerSecondMagnitude - decreaseValue).toInt256() *
                    sign;
            }
        }
        ...
        //nextSavedFundingFactorPerSecond应该在 0 和 maxFundingFactorPerSecond之间
    ...
    function updateCumulativeBorrowingFactor(
        ...
        incrementCumulativeBorrowingFactor(dataStore, eventEmitter, market.marketToken, isLong, delta);

        dataStore.setUint(
            Keys.cumulativeBorrowingFactorUpdatedAtKey(market.marketToken, isLong),
    ...
    // @return (pnl of positions) / (long or short pool value)
    // 获取 pnl /  pool 价值
    //如果  maximize也为 true  poolUsd取小值 pnl pnl取大值  ==>pnl/poolUsd 绝对值变大
    //      - pnl 小于0  pnl/poolUsd变小
    //      - pnl 大于0  pnl/poolUsd变大
    //如果  maximize也为 false  poolUsd取大值    pnl取小值  ==>pnl/poolUsd 绝对值变小
    function getPnlToPoolFactor(
        DataStore dataStore,
        Market.Props memory market,
        MarketPrices memory prices,
        ...
        //获取一个池子的代币的价值观 注意是一个代币 通过 Long 来判断
        //
    ...
    function validateOpenInterest(DataStore dataStore, Market.Props memory market, bool isLong) internal view {
    ...
    // @param market the market to check
    // @param token the token to check
    function validatePoolAmount(DataStore dataStore, Market.Props memory market, address token) internal view {
        uint256 poolAmount = getPoolAmount(dataStore, market, token);
    ...
    function getVirtualInventoryForSwaps(
        DataStore dataStore,
        address market
    ...
    function applyDeltaToVirtualInventoryForSwaps(
        ...
        //返回 bool 值来判断是longToken 还是 short Token
        ...
        MarketEventUtils.emitVirtualSwapInventoryUpdated(
            eventEmitter,
            market.marketToken,
            isLongToken,
            virtualMarketId,
    ...
    function applyDeltaToVirtualInventoryForPositions(
        ...
        int256 nextValue = dataStore.applyDeltaToInt(Keys.virtualInventoryForPositionsKey(virtualTokenId), delta);
    ...
    // @param shortToken the short token of the market
    // 获取市场所有的size
    function getOpenInterest(DataStore dataStore, Market.Props memory market) internal view returns (uint256) {
    ...
    function getMaxPositionImpactFactor(
        DataStore dataStore,
        address market,
        bool isPositive
    ) internal view returns (uint256) {
        (uint256 maxPositiveImpactFactor, uint256 maxNegativeImpactFactor) = getMaxPositionImpactFactors(
            dataStore,
            market
        );
    ...
    // @param dataStore DataStore
    // @param market the market to check
    function getMaxPositionImpactFactorForLiquidations(
        DataStore dataStore,
        address market
    ) internal view returns (uint256) {
    ...
    function getMinCollateralFactorForOpenInterestMultiplier(
        DataStore dataStore,
        address market,
        bool isLong
    ) internal view returns (uint256) {
    ...
    function getMinCollateralFactorForOpenInterest(
        ...
        uint256 multiplierFactor = getMinCollateralFactorForOpenInterestMultiplier(
            dataStore,
            market.marketToken,
            isLong
        );
    ...
    function getOpenInterestReserveFactor(
        DataStore dataStore,
        address market,
        bool isLong
    ) internal view returns (uint256) {
    ...
    function setSavedFundingFactorPerSecond(
        DataStore dataStore,
        address market,
        int256 value
    ) internal returns (int256) {
    ...
    function getFundingFeeAmountPerSize(
        DataStore dataStore,
        address market,
        address collateralToken,
        bool isLong
    ...
    function getClaimableFundingAmountPerSize(
        DataStore dataStore,
        address market,
        address collateralToken,
        bool isLong
    ...
    function getBorrowingExponentFactor(
        DataStore dataStore,
        address market,
        bool isLong
    ) internal view returns (uint256) {
    ...
    function getCumulativeBorrowingFactor(
        DataStore dataStore,
        address market,
        bool isLong
    ) internal view returns (uint256) {
    ...
    function incrementCumulativeBorrowingFactor(
        ...
        MarketEventUtils.emitBorrowingFactorUpdated(eventEmitter, market, isLong, delta, nextCumulativeBorrowingFactor);
    }
    ...
    function getCumulativeBorrowingFactorUpdatedAt(
        DataStore dataStore,
        address market,
        bool isLong
    ) internal view returns (uint256) {
    ...
    // 获上次更新borrow factor到现在经历的多少秒
    function getSecondsSinceCumulativeBorrowingFactorUpdated(
        DataStore dataStore,
        address market,
        bool isLong
    ) internal view returns (uint256) {
    ...
    function getNextCumulativeBorrowingFactor(
        ...
        //
        uint256 durationInSeconds = getSecondsSinceCumulativeBorrowingFactorUpdated(
            dataStore,
            market.marketToken,
            isLong
        );
    ...
    function getNextPositionImpactPoolAmount(DataStore dataStore, address market) internal view returns (uint256) {
        (
            ,
    ...
    function getPendingPositionImpactPoolDistributionAmount(
        ...
        //todo 只有交易对position 产生ImpactPoolAmount 才会有 distributionAmount
    ...
    function getSwapPathMarkets(
        DataStore dataStore,
        address[] memory swapPath
    ) internal view returns (Market.Props[] memory) {
    ...
    function validateMaxPnl(
        ...
        (
            bool isPnlFactorExceededForLongs,
            int256 pnlToPoolFactorForLongs,
            uint256 maxPnlFactorForLongs
        ) = isPnlFactorExceeded(dataStore, market, prices, true, pnlFactorTypeForLongs);
        ...
        (
            bool isPnlFactorExceededForShorts,
            int256 pnlToPoolFactorForShorts,
            uint256 maxPnlFactorForShorts
        ) = isPnlFactorExceeded(dataStore, market, prices, false, pnlFactorTypeForShorts);
    ...
    function isPnlFactorExceeded(
        ...
        return isPnlFactorExceeded(dataStore, _market, prices, isLong, pnlFactorType);
    }
    ...
    function isPnlFactorExceeded(
        ...
        //获取 pnl 和池子资金的比例 maximize为 true pnlToPoolFactor的绝对值 变大
        //为什么设置maximize为true - 会导致pnlToPoolFactor的值取最大 更符合平台利益
    ...
    function setUiFeeFactor(
        ...
        dataStore.setUint(Keys.uiFeeFactorKey(account), uiFeeFactor);

        MarketEventUtils.emitUiFeeFactorUpdated(eventEmitter, account, uiFeeFactor);
    }

    function validateMarketTokenBalance(DataStore dataStore, Market.Props[] memory markets) public view {
        for (uint256 i; i < markets.length; i++) {
            validateMarketTokenBalance(dataStore, markets[i]);
        }
    }

    function validateMarketTokenBalance(DataStore dataStore, address _market) public view {
    ...
    function validateMarketTokenBalance(DataStore dataStore, Market.Props memory market, address token) internal view {
        ...
        if (balance < collateralAmount) {
            revert Errors.InvalidMarketTokenBalanceForCollateralAmount(
                market.marketToken,
                token,
                balance,
                collateralAmount
        ...
        uint256 claimableFundingFeeAmount = dataStore.getUint(
            Keys.claimableFundingAmountKey(market.marketToken, token)
        );
        ...
        if (balance < claimableFundingFeeAmount) {
            revert Errors.InvalidMarketTokenBalanceForClaimableFunding(
                market.marketToken,
                token,
                balance,
                claimableFundingFeeAmount
            );
        }
    }
    ...
    function getExpectedMinTokenBalance(
        ...
        cache.claimableCollateralAmount = dataStore.getUint(
            Keys.claimableCollateralAmountKey(market.marketToken, token)
        );
